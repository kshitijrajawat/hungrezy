    ...
    API.get(`/product/get-product/${slug}`)
      .then(({ data }) => {
        ...
        if (data.product?.id && data.product?.category?.id) {
          API.get(`/product/related-product/${data.product.id}/${data.product.category.id}`)
      ...
      >
        ...
        <div className="card overflow-hidden">
          <img
            ...
            onError={(e) => { e.target.src = "https://placehold.co/600x400/1a1a1a/f97316?text=Food"; }}
          />
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-6 mb-8">
            <span className="text-4xl font-bold text-brand-400">₹{product.price}</span>
            <span className={`badge ${product.quantity > 0 ? "bg-green-500/10 text-green-400 border-green-500/30" : "bg-red-500/10 text-red-400 border-red-500/30"} border`}>
          ...
          <button
            ...
            className={`flex items-center gap-2 ${inCart ? "btn-outline cursor-default" : "btn-primary"}`}
          >
            <FiShoppingCart />
